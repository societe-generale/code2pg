...
Prints a short help.
With a parameter, prints help on the specified RDBMS instruction, such as 
links to official documentation, comments, difficulty ...

=item B<-l> , B<--language> string

The chosen language calling the RDBMS. This should be set together with 
I<extension> when no configuration file is provided.
...
Migration will be from the specified RDBMS to PostgreSQL. So far, only 
Oracle is supported. 
