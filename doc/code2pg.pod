...
code2pg will estimate in man-days how difficult a migration
from Oracle to PostgreSQL would be regarding application code. It does not
handle the database code as ora2pg does, only the given application code.
Therefore, a directory of source code (either files or svn) should be given as
input and code2pg will generate a report in one of the
following format: minimal, text or html.
...
Oracle keywords will be redirected to this csv file.
...
like PGGREPAPPCODE_OracleInstruction, which should allow easy searching. As for
